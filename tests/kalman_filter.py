...
ekf = ExtendedKalmanFilter(f, h, A, B, H, Q, R, initial_state, initial_covariance)
...
for i in range(num_steps):
    true_state = f(true_states[i-1]) + np.sqrt(Q) @ np.random.randn(1, 1)
    measurement = h(true_state) + np.sqrt(R) @ np.random.randn(1, 1)
